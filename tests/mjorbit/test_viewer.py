...
from mjorbit import ControlMomentGyroSpec, mjo_forward, mjo_step
from mjorbit.testdata import FREE_BODY_XML, TWO_BODIES_XML
from tests.mjorbit._helpers import make_model_data, set_freejoint_lvlh_state
...
def test_viewer_eci_render_is_chief_centered() -> None:
    # The "eci" view uses a chief-centered floating origin: bodies render near
    # the scene origin (scaled local offsets) and the Earth is translated by
    # -R_eci instead. Placing metre-scale geometry at absolute ECI (~6.8e6 m)
    # caused float32 jitter and camera drift; guard against that regression.
    _require_viewer_server()
    model, data = make_model_data(xml_path=FREE_BODY_XML)
    viewer = MjOrbitViewer(
        model,
    ...
    try:
        viewer.set_local_scene_scale(10.0)
        expected = 10.0 * data.xpos[1]
        np.testing.assert_allclose(
            np.asarray(viewer.mj_scene._body_frames[0].position),
            expected,
        )
        # The body must stay near the origin, far from absolute ECI coordinates.
        assert np.linalg.norm(viewer.mj_scene._body_frames[0].position) < 1.0e3
    finally:
        viewer.close()
