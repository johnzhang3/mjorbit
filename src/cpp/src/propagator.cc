...
void derivatives(
  ...
  total_accel(R, out_dV, use_j2, central_body);
  if (a_external) {
...
void propagate_rk4(
  ...
  double k1R[3];
  double k1V[3];
  derivatives(R_eci, V_eci, use_j2, a_external, central_body, k1R, k1V);
  ...
  double k2R[3];
  double k2V[3];
  derivatives(R_tmp, V_tmp, use_j2, a_external, central_body, k2R, k2V);

  detail::add_scaled3(R_eci, k2R, 0.5 * dt, R_tmp);
  detail::add_scaled3(V_eci, k2V, 0.5 * dt, V_tmp);
  double k3R[3];
  double k3V[3];
  derivatives(R_tmp, V_tmp, use_j2, a_external, central_body, k3R, k3V);

  detail::add_scaled3(R_eci, k3R, dt, R_tmp);
  detail::add_scaled3(V_eci, k3V, dt, V_tmp);
  double k4R[3];
  double k4V[3];
  derivatives(R_tmp, V_tmp, use_j2, a_external, central_body, k4R, k4V);
