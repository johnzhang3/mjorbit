...
from mjorbit.constants import GM_EARTH, J2_EARTH, R_EARTH
...
@wp.func
def _j2_accel_f64(R: wp.vec3d) -> wp.vec3d:
    """float64 J2 acceleration, used only for the cancellation-prone body-relative
    differential (see ``_relative_accel``)."""
    one = wp.float64(1.0)
...
def _relative_accel(
    ...
    """Differential gravity ``g(R+rho) - g(R)``: Encke for the point-mass term;
    the J2 term is differenced in float64.

    A literal float32 ``_j2_accel(R+rho) - _j2_accel(R)`` catastrophically
    cancels: the cancellation depends on ``||rho|| / ||R_chief||`` (~1e-7 for
    metre-scale bodies at ~7000 km), which is at float32 epsilon, regardless of
    J2's small magnitude. We therefore form ``R_chief + rho`` and the difference
    in float64 (so ``rho`` is not lost to float32 rounding) and cast the small
    result back. The point-mass Encke form is already cancellation-safe."""
    a = _encke_point_mass_relative_accel(rho, R_chief)
    if use_j2 != 0:
        rc = wp.vec3d(wp.float64(R_chief[0]), wp.float64(R_chief[1]), wp.float64(R_chief[2]))
        rp = wp.vec3d(
            wp.float64(R_chief[0]) + wp.float64(rho[0]),
            wp.float64(R_chief[1]) + wp.float64(rho[1]),
            wp.float64(R_chief[2]) + wp.float64(rho[2]),
        )
        dj = _j2_accel_f64(rp) - _j2_accel_f64(rc)
        a = a + wp.vec3(wp.float32(dj[0]), wp.float32(dj[1]), wp.float32(dj[2]))
    return a
@wp.func
