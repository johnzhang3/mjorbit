...
``mjorbit_warp.step(model, data)`` like MJWarp; when ``model`` and ``data``
...
def _restore_public_aliases() -> None:
    # Importing the internal ``.step`` package sets ``mjorbit_warp.step``
...
def put_model(model: Any) -> Any:
    ...
    if isinstance(model, CpuMjoModel):
        cached = getattr(model, "_mjorbit_warp_model", None)
        if cached is not None:
            return cached
        wrapped = MjoModel.from_host_model(model)
        setattr(model, "_mjorbit_warp_model", wrapped)
        return wrapped
...
def put_data(model: Any, data: Any, *args: Any, **kwargs: Any) -> Any:
    ...
    if isinstance(model, CpuMjoModel):
        if not isinstance(data, CpuMjoData):
            raise TypeError("orbit-aware put_data requires a mjorbit.MjoData instance")
...
def __getattr__(name: str) -> Any:
    if name not in _MJWARP_PUBLIC_NAMES:
        raise AttributeError(f"module 'mjorbit_warp' has no attribute {name!r}")
