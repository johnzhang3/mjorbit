"""Spline-knot MPPI planner on top of mjorbit's batched rollout.
...
The control vector follows ``mjorbit.rollout``'s layout: ``model.nu``
...
from mjorbit import MjoData, MjoModel, mjo_forward
from mjorbit.rollout import mjo_control_size, mjo_get_state, mjo_set_state, rollout
...
class MppiPlanner:
    ...
    def action(self, time: float) -> np.ndarray:
        ...
        return self._clip_controls(np.asarray(self.spline(time), dtype=np.float64))

    def _clip_controls(self, controls: np.ndarray) -> np.ndarray:
        """Clamp interpolated controls to the configured ``[ctrl_low, ctrl_high]``.

        Clamping only the spline knots is insufficient: a cubic interpolant
        overshoots the box between clamped knots, so the per-step controls (and
        the executed ``action``) must be clamped too.
        """
        if self._ctrl_low is None and self._ctrl_high is None:
            return controls
        return np.clip(controls, self._ctrl_low, self._ctrl_high)
    ...
    def update_action(self, data: MjoData) -> np.ndarray:
        ...
        them out with ``mjorbit.rollout``, reweights, and rebuilds the
        ...
        # Re-clamp the interpolated controls: clipping the knots alone lets a
        # cubic spline overshoot the box between them.
        controls = self._clip_controls(
            np.asarray(
