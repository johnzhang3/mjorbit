...
def main() -> None:
    ...
    com_1_eci = np.average(
        [mjd.xipos[i] for i in range(1, mjm.nbody)],
        weights=[mjm.body_mass[i] for i in range(1, mjm.nbody)],
        axis=0,
    )
    com_1 = data.lvlh_position_from_eci(com_1_eci)
    ...
    for i in range(n_steps_2):
        ...
        if i % 500 == 0:
            ...
            ee_pos = data.lvlh_position_from_eci(mjd.xipos[ee_id])
            ee_positions.append(ee_pos)

    ee_id = model.body_id("ee")
    ee_final = data.lvlh_position_from_eci(mjd.xipos[ee_id])
    print(
